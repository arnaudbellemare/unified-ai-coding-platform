...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!extractorType || !text) {
      return NextResponse.json({ error: 'Missing extractorType or text' }, { status: 400 })
    }

    if (!['geo_query', 'acp_payment', 'ap2_agent', 'document_metadata'].includes(extractorType)) {
      return NextResponse.json({ error: 'Invalid extractorType' }, { status: 400 })
    }

    const result = await langStructService.extract(extractorType as ExtractorType, text)
