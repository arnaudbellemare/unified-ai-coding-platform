    ...
    const messages = [
      {
        role: 'system',
        content:
          'You are an expert programmer. Generate clean, efficient, and well-commented code. Always provide working code that can be executed.',
      },
      {
        role: 'user',
        content: prompt,
...
`
        ...
        const writeResult = await runCommandInSandbox(sandbox, 'sh', [
          '-c',
          `echo '${script.replace(/'/g, "'\"'\"'")}' > generated-code.js`,
        ])
    ...
    // Process payment
    const payment = await auth.processPayment(userSession.userId, result.cost, `OpenRouter vibe code: ${modelId}`)
