...
export class HybridOptimizer {
  private promptOptimizerStrategies = ['entropyOptim', 'synonymReplace', 'lemmatization', 'punctuationOptim']
    ...
    switch (strategy) {
      case 'entropyOptim':
        ...
        optimized = prompt.replace(
          /\b(the|a|an|and|or|but|in|on|at|to|for|of|with|by|is|are|was|were|be|been|being)\b/gi,
          '',
        )
  ...
  private async optimizeWithCAPOEnhanced(
    ...
    const capoResult = await capoEnhancedOptimizer.optimizeWithCAPO(prompt, taskDescription, fewShotExamples)
   ...
   */
  async getOptimizationRecommendations(prompt: string, taskDescription: TaskDescription): Promise<string[]> {
    const verbosityLevel = this.determineVerbosityLevel(prompt, taskDescription)
  ...
  async batchOptimize(
    ...
    for (const item of prompts) {
      const result = await this.optimize(item.prompt, item.taskDescription, item.fewShotExamples || [])
      results.push(result)
    }
