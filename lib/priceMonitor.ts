...
export class PriceMonitor {
  ...
  private async updateProviderPrice(providerId: string, providerName: string) {
    try {
      ...
      if (Math.abs(newPrice - currentPrice) / currentPrice > 0.05) { // 5% change
        ...
        console.log(`Price update: ${providerName} - $${currentPrice.toFixed(4)} → $${newPrice.toFixed(4)} (${priceUpdate.changePercentage.toFixed(2)}%)`);
      }
    } catch (error) {
  ...
  async getCurrentPrices(): Promise<Array<{
    providerId: string;
    providerName: string;
    currentCost: number;
    previousCost: number;
  ...
  private checkPriceAlerts(priceUpdate: PriceUpdate) {
    const alerts = this.priceAlerts.filter(
      alert => alert.providerId === priceUpdate.providerId && alert.isActive
    );
    ...
    for (const alert of alerts) {
      ...
      if (triggered) {
        ...
        console.log(`Price alert triggered: ${alert.id} - ${priceUpdate.providerName} ${alert.condition} ${alert.threshold}`);
  ...
  async getOptimizationRecommendations(): Promise<string[]> {
    ...
    recommendations.push(`💰 Cheapest option: ${cheapest.providerName} at $${cheapest.currentCost.toFixed(4)} per request`);
    
    if (significantChanges.length > 0) {
      significantChanges.forEach(change => {
        const direction = change.changePercentage > 0 ? 'increased' : 'decreased';
        recommendations.push(`📈 ${change.providerName} price ${direction} by ${Math.abs(change.changePercentage).toFixed(1)}%`);
      });
    }
