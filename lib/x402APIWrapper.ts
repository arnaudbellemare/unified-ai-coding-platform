...
export class X402APIWrapper {
  ...
  getCheapestProvider(): Provider | null {
    ...
    return activeProviders.reduce((cheapest, current) => (current.cost < cheapest.cost ? current : cheapest))
  }
  ...
  getBestPerformingProvider(): Provider | null {
    ...
    return activeProviders.reduce((best, current) => (current.performance > best.performance ? current : best))
  }
